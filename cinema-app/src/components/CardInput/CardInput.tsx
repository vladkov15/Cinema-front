...
const CardInput: React.FC<CardInputProps> = ({ label }) => {
  const { register, formState: { errors } } = useForm<CardData>();
  ...
  return (
    <div className={styles['card-input']}>
        ...
        />
        {errors.cardHolderName && <div className={styles.error}>Please enter the cardholder name</div>}
      </div>
      <div className={styles['card-number']}>
        ...
        />
        {errors.cardNumber && <div className={styles.error}>Please enter a valid card number</div>}
      </div>
